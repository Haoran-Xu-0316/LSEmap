...
for (const code of ['PAR']) {
  for (const width of [1440, 390]) {
    test(`${code} stepped hall and entrance stay usable at ${width}px`, async ({ page }) => {
      const errors = [];
      ...
      await page.waitForTimeout(1100);
      await page.screenshot({ path: `result/web/local10/${code.toLowerCase()}-exterior-${width}.png` });
