...
checks = {
    ...
    'four_exteriors_rebuilt': {item['code'] for item in review if item['replacedObjects']} == {'KSW','50L','51L','SAR'},
    'all_31_buildings_recorded': len(review) == len({item['code'] for item in review}) == 31,
    'eighteen_independent_samples': len(rooms) == len({item['code'] for item in rooms}) == 18,
    'twenty_three_interiors': sum(bool(collection.all_objects) for collection in bpy.data.collections if collection.name.endswith('_PUBLIC_INTERIOR_study')) == 23,
...
print('EDITION20_NATIVE_AUDIT_PASS', json.dumps(report), flush=True)
