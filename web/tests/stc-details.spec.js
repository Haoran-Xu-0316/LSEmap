...
for (const code of ['STC']) {
  for (const width of [1440, 390]) {
    test(`${code} recessed facade and entrance stay usable at ${width}px`, async ({ page }) => {
      const errors = [];
      ...
      await page.screenshot({ path: `result/web/local11/${code.toLowerCase()}-entrance-${width}.png` });
      ...
      await page.waitForTimeout(1100);
      await page.screenshot({ path: `result/web/local11/${code.toLowerCase()}-exterior-${width}.png` });
