...
checks = {
    ...
    'nineteen_independent_samples': len(rooms) == len({item['code'] for item in rooms}) == 19,
    'twenty_four_interiors': sum(bool(collection.all_objects) for collection in bpy.data.collections if collection.name.endswith('_PUBLIC_INTERIOR_study')) == 24,
...
print('EDITION21_NATIVE_AUDIT_PASS', json.dumps(report), flush=True)
