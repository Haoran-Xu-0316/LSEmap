...
test('release manifest identifies every model and gallery in edition 16', async () => {
  ...
  expect(release.version).toBe('16');
...
test('gallery images use a revisioned URL to bypass the previous release cache', async ({ page }) => {
  ...
  await expect(page.locator('.detail-photo img')).toHaveAttribute('src', /\/images\/mar-exterior\.webp\?v=16-[a-f0-9]{12}$/);
  await page.locator('.detail-photo').click();
  await expect(page.locator('#gallery-image')).toHaveAttribute('src', /\?v=16-[a-f0-9]{12}$/);
