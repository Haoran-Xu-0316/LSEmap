"""Render original version-15 MAR entrance study for public gallery thumbnails.
...
OUT = ROOT / 'result/blender/stage15/mar/renders'
OUT.mkdir(parents=True, exist_ok=True)
bpy.ops.wm.open_mainfile(filepath=str(ROOT / 'result/blender/LSE_campus_detailed_v15.blend'))
# Earlier galleries remain unchanged; render the locally refined Marshall Building.
records = json.loads((ROOT / 'result/blender/stage15/mar/mar-manifest.json').read_text())['buildings']
for record in records:
    code = record['code']
    scene = bpy.data.scenes.new('RENDER_INFILL_'+code)
    scene.collection.children.link(bpy.data.collections[code+'_EXTERIOR'])
    scene.collection.children.link(bpy.data.collections[code+'_PUBLIC_INTERIOR_study'])
    ...
    normal = Vector((direction[0],-direction[2],-.08)).normalized()
