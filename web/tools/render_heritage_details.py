"""Render original version-09 heritage studies for public gallery thumbnails.
...
OUT = ROOT / 'result/blender/stage09/heritage/renders'
OUT.mkdir(parents=True, exist_ok=True)
bpy.ops.wm.open_mainfile(filepath=str(ROOT / 'result/blender/LSE_campus_detailed_v09.blend'))
# Earlier galleries remain unchanged; render the locally refined Cowdray and King's Chambers elevations.
records = json.loads((ROOT / 'result/blender/stage09/heritage/heritage-manifest.json').read_text())['buildings']
for record in records:
    ...
    entrance = Vector(((wall['p'][0]+wall['q'][0])/2,(wall['p'][1]+wall['q'][1])/2,3.05))
    camera.location = entrance + Vector((direction[0],-direction[2],.10)).normalized()*35
