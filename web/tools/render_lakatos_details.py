"""Render original version-13 Lakatos study for public gallery thumbnails.
...
OUT = ROOT / 'result/blender/stage13/lakatos/renders'
OUT.mkdir(parents=True, exist_ok=True)
bpy.ops.wm.open_mainfile(filepath=str(ROOT / 'result/blender/LSE_campus_detailed_v13.blend'))
# Earlier galleries remain unchanged; render the locally refined Lakatos elevations.
records = json.loads((ROOT / 'result/blender/stage13/lakatos/lakatos-manifest.json').read_text())['buildings']
for record in records:
    ...
    normal = Vector((direction[0],-direction[2],.20)).normalized()
    ...
    profile = next(p for p in json.loads((ROOT/'result/blender/stage05/infill-geometry.json').read_text()) if p['code']==code)
    wall = next(w for w in profile['walls'] if w['front'])
    entrance = Vector(record['detailView']['target'])
    entry_direction = Vector(record['detailView']['position']) - entrance
    camera.location = entrance + entry_direction.normalized()*35
    camera.rotation_euler = (entrance-camera.location).to_track_quat('-Z','Y').to_euler()
    camera_data.ortho_scale = 8.5
    scene.render.resolution_x=1100;scene.render.resolution_y=1100
    scene.render.filepath=str(OUT/(code.lower()+'-windows.png'))
    bpy.ops.render.render(write_still=True)
    print('RENDERED_ENTRANCE',code,flush=True)
