...
FINISH_RECORDS = {b['code']: b for b in json.loads((ROOT / 'result/blender/stage16/all-buildings-manifest.json').read_text())['buildings']}
material_cache = {}
...
def clone_group(objects, name, target_scene):
    ...
    for original in objects:
        ...
        if not full_detail and '_V16_' in original.name and not original.name.startswith('35L_'):
            continue
...
payload = {
    'version': '16', 'sourceModelSha256': hashlib.sha256(MODEL.read_bytes()).hexdigest(),
